...
for n in ${containerList[@]};
do
	containerNeedsRestart=0
	...
	if [ -f "$github_trading_settings_filename" ]; then
		...
		if [ $curr_sum -eq $downloaded_sum ]; then
			...
			# Flag container for restart if not already flaged.
			if [ ! $containerNeedsRestart -eq 1]; then
			containerRestartList=1	
			fi
	...
	# Restart docker container if neccessary.
	if [ $containerNeedsRestart -eq 1]; then
